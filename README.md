...
The .exe should be a runnable standalone program. If the .exe runs too slowly, or Windows flags the programs as a false positive, you will need to install Python:
...
3. Press "Get"
4. Once the application downloads, press Windows Key + R -> type "CMD" (without the quotes) -> press enter (alternatively, press the search bar in the task bar and look for "CMD" -> press on "Command Prompt")
5. When the command line opens, type (without quotations) "Python -m pip install pymupdf PyQt6 Pillow" (this should install all the necessary dependencies) and press enter
6. run the program by double clicking on WatermarkRemover.py
...
3. If you see information about the package, it has been installed correctly. If instead you see "WARNING: Package(s) not found: pymupdf" proceed to the next step
4. In the command line, type "Python -m pip install pymupdf"
5. confirm the package was installed by repeating step 2
